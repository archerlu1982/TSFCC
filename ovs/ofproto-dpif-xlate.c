...
static void
compose_set_rwnd_action(struct xlate_ctx *ctx, struct ofpact_set_rwnd *setRwnd)
{
    uint16_t rwnd = setRwnd->rwnd;

    nl_msg_put_u16(ctx->odp_actions, OVS_ACTION_ATTR_SET_RWND, rwnd);
}
    ...
    OFPACT_FOR_EACH (a, ofpacts, ofpacts_len) {
        ...
        case OFPACT_SET_RWND:
            if (is_ip_any(flow) && !(flow->nw_frag & FLOW_NW_FRAG_LATER)) {
                compose_set_rwnd_action(ctx, ofpact_get_SET_RWND(a));
            }
            break;
        case OFPACT_SET_VLAN_VID:
            wc->masks.vlans[0].tci |= htons(VLAN_VID_MASK | VLAN_CFI);

...
static int set_tcp_rwnd(struct sk_buff *skb, struct sw_flow_key *flow_key, uint16_t rwnd)
{
	struct tcphdr *th;
	__be16 window;
	int err;
	...
	window = htons(rwnd);
	if (likely(rwnd < ntohs(th->window))) {
			      ...
			      const struct nlattr *attr, int len)
{
	...
	for (a = attr, rem = len; rem > 0;
		...
		case OVS_ACTION_ATTR_SET_RWND:
			err = set_tcp_rwnd(skb, key, nla_get_u16(a));
			break;

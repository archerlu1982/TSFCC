...
int send_ack_userspace_packet(struct datapath *dp, uint32_t qlen, char name[32], bool flag)
{
    struct ovs_header *upcall;
    struct sk_buff *user_skb = NULL; /* to be queued to userspace */    
    size_t len;
    ...
    dp_ifindex = get_dpifindex(dp);
    if (!dp_ifindex){
        printk("---send_ack_userspace_packet !dp_if---");
        return -ENODEV;
    }
    sscanf(name, "s%u-eth%u", &datapath_id, &port);
	if(datapath_id != 0){
		portid = datapath_id * 234;
	}
    // printk("------------send_ack_userspace_packet------start-------");
    // printk("----send_ack_userspace_packet--datapath:%p-%llu--------",dp,qlen);
    ...
    len = NLMSG_ALIGN(sizeof(struct ovs_header)) + nla_total_size(sizeof(uint32_t)) + nla_total_size(sizeof(uint32_t)) + 
            nla_total_size(sizeof(uint32_t)) + nla_total_size(sizeof(bool));

    user_skb = genlmsg_new(len, GFP_ATOMIC);
    if (!user_skb) {
        err = -ENOMEM;
        printk("---send_ack_userspace_packet !user_skb---");
        goto out;
    }
    ...
    if (nla_put_u32(user_skb, OVS_ACK_ATTR_DATAPATH_ID,
            datapath_id)) {
        err = -ENOBUFS;
        printk("---send_ack_userspace_packet !frontDpid---");
        goto out;
    }
    ...
    if(nla_put_u32(user_skb,OVS_ACK_ATTR_PORT_NUM,
            port)){
        err = -ENOBUFS;
        printk("---send_ack_userspace_packet !hashid---");
        goto out;
    }

    /* Add OVS_PACKET_ATTR_TYPE_ID */
    if(nla_put_u32(user_skb,OVS_ACK_ATTR_QUEUE_LENGTH,
            qlen)){
        err = -ENOBUFS;
        printk("---send_ack_userspace_packet !typeid---");
        goto out;
    }
    ...
    if (nla_put_u8(user_skb, OVS_ACK_ATTR_CONGESTION_FLAG,
            flag)) {
        err = -ENOBUFS;
        printk("---send_ack_userspace_packet !packetid---");
        goto out;
    }
    ...
    if(ovs_dp_get_net(dp) == NULL){
        printk("-----send_ack_userspace_packet ovs_dp_get_net(dp) err-----------");
    }
    err = genlmsg_unicast(ovs_dp_get_net(dp), user_skb, portid);
    user_skb = NULL;
out:
    // kfree_skb(user_skb);
    // printk("------------send_ack_userspace_packet------end-------");
    return err;    
}
